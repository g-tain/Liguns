...
export function JobFormModal({ isOpen, onClose, jobToEdit, onSuccess }: JobFormModalProps) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const filePath = `jobs/${Date.now()}-${safeBase}.${ext}`;
      ...
      toast.error("Gagal upload logo", { description: error.message });
    } finally {
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-12">
           ...
           <div className="space-y-6">
              <div className="flex items-center gap-3">
                 ...
                 <h3 className="text-xs font-black text-white/60 uppercase tracking-widest">Media</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="space-y-3">
                    <label className="text-[10px] text-gold uppercase tracking-tighter ml-4">Upload Logo Venue</label>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 font-jost text-xs text-white"
                    />
                    <p className="text-[10px] text-white/40 ml-4">Upload langsung ke bucket `talent-assets`.</p>
                 </div>

                 <div className="space-y-3">
                    <label className="text-[10px] text-gold uppercase tracking-tighter ml-4">URL Logo (Opsional)</label>
                    <input
                 ...
                 {formData.image_url && (
                   <div className="md:col-span-2 flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4">
                      ...
                      <div className="flex-1">
                         <p className="text-xs text-white/70 font-black uppercase tracking-widest">Preview Logo</p>
                         <p className="text-[10px] text-white/40 break-all">{formData.image_url}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, image_url: "" }))}
                        className="px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest bg-red-500/10 text-red-400"
                      >
                        Hapus
                      </button>
                   </div>
